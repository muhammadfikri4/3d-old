...
export const useGeo = () => {
  ...
  const map = new mapboxgl.Map({
    ...
    center: [103.85233599700446, 1.284132369245514],
    zoom: 20,
    pitch: 45,
    bearing: -17.6,
    container: "map",
  ...
  //   $(".select-floor").click();
  const selectFloor = document.querySelector(".select-floor");
  selectFloor.addEventListener("click", function (ev) {
    currentIndex = parseInt(ev.currentTarget.getAttribute("tabIndex"));
    toggle(currentIndex);
  });

  function toggle(levelIndex) {
    map.setFilter("room-extrusion", [
      "==",
      ["to-number", ["get", "levelIndex"]],
      levelIndex,
    ]);
  }
  // --------------------------------------------------------------------------------------------
  ...
  map.on("load", () => {
    ...
    map.on("click", "room-extrusion", (e) => {
      ...
      new mapboxgl.Popup()
        .setLngLat(e.lngLat)
        .setHTML(
          `<h3>${properties.name}</h3><p>name: ${properties.name}<br />Type: ${properties.type}<br />ID: ${properties.id}</p>`
        )
        .addTo(map);
    });
